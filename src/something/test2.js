...
const { cookieSet } = require("./cookie/cookieSet");
...
(async () => {
  ...
  const producer = async (d) => {
    ...
    for (let i = 0; i < len; i++) {
      ...
      /* 检测前后图片尺寸是否匹配 */
      const dimensionsBefore = sizeOf(`src/image/${d.id}before${i}.png`);
      const dimensionsAfter = sizeOf(`src/image/${d.id}after${i}.png`);
  ...
  const Shot = async (data) => {
    ...
    for (let index = 0; index < n; index++) {
      ...
      await page.screenshot({
        path: `src/image/${data.id}after${index}.png`,
        fullPage: true,
      });
    }
    await browser.close();
